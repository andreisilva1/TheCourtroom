...
import type {
  ...
  StartDebateResponse, ArgueResponse, ObjectionResponse, DebateHistoryEntry, DebateDetails,
} from '../types'
...
export async function loadDebateDetails(debateId: string): Promise<DebateDetails> {
  const { data } = await api.get<DebateDetails>(`/debates/${debateId}/details`)
  return data
}
